...
const string strMessageMagic = "twenty15 Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 336)
    {
        nSubsidy = 10000 * COIN;
    }
    else if(pindexBest->nHeight < 672)
    {
        nSubsidy = 9000 * COIN;
    }
    else if(pindexBest->nHeight < 1008)
    {
        nSubsidy = 8000 * COIN;
    }
    else if(pindexBest->nHeight < 1344)
    {
        nSubsidy = 7000 * COIN;
    }
    else if(pindexBest->nHeight < 1680)
    {
        nSubsidy = 6000 * COIN;
    }
    else if(pindexBest->nHeight < 2015)
    {
        nSubsidy = 5000 * COIN;
    }
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, nStakeModifierChecksum=0x%016"PRIx64, pindexNew->nHeight, pindexNew->nStakeModifierChecksum);
...
// twenty15: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "twenty15", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "'Blood on Many Hands': Police union president slams De Blasio after cops' killing - Published December 21, 2014FoxNews.com";
        CTransaction txNew;
        txNew.nTime = 1419147574;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1419147574;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1442361;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xb4af916dfecf8cc62f9f7f0df0319e4ae85d8cd619316155916f610a8e7c6c27"));
        block.print();
